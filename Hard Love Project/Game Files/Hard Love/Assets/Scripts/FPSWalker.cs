...
public class FPSWalker: Photon.MonoBehaviour {
    ...
    public float observedHeight = 2;
    public bool observedCrouching = false;
	...
	void FixedUpdate() {
        ...
        if (canCrouch) {
            ...
            if (photonView.isMine)
            {
                if (canControl && Input.GetButton("Crouch"))
                {
                    h = 1f;
    ...
    void Update()
    {
        ...
        if (!toggleWalk) {
            if (canControl && (Input.GetButton("Walk") || crouching == true))
            {
                speed = walkSpeed;
                walking = true;
                running = false;
        ...
        //if there is walk toggle, this switches between walkActivated mode and non walkActivated modes
            ...
            if (Input.GetButtonDown("Walk") && canControl)
                {
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        else
        {
            ...
            observedHeight = (float)stream.ReceiveNext();
            observedCrouching = (bool)stream.ReceiveNext();
        }
